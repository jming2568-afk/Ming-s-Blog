...
//   2) 如果 TRAE_WORKSPACE 存在（当前沙箱会设置，指向项目根目录），优先写到 <TRAE_WORKSPACE>/data，
//      避免 Vercel/Lambda 风格运行时把 cwd 切到 /var/task 这种一次性/只读目录。
//   3) 若 process.cwd() 本身可写且不是 /var/task（一次性任务目录），则退回到 <cwd>/data。
//   4) 其它情况（cwd 不可写、或者 cwd === /var/task）兜底写 /tmp/<project>/data，保证预览也能跑。
function computeDefaultDbPath() {
  const workspace = process.env.TRAE_ENV_WORKSPACE || process.env.TRAE_WORKSPACE;
  if (workspace) {
    return path.join(workspace, "data", "portfolio.db");
  }
  const cwd = process.cwd();
  const cwdUsable =
    cwd !== "/var/task" &&
    !path.resolve(cwd).startsWith("/var/task") &&
    isDirWritable(cwd);
  if (cwdUsable) return path.join(cwd, "data", "portfolio.db");
  // 兜底：/tmp 下建一个子目录，避免和其它任务冲突
  const slug = (process.env.npm_package_name || "portfolio").replace(/[^a-zA-Z0-9_-]/g, "_") || "portfolio";
  return path.join("/tmp", slug, "data", "portfolio.db");
}
...
function buildInitError(err) {
  ...
  const looksServerlessCwd =
    process.cwd() === "/var/task" ||
    DB_PATH.startsWith("/var/task/") ||
    /\bvar\/task\b/.test(DB_PATH);
  if (/unable to open database file|ENOENT|no such file/i.test(msg) && looksServerlessCwd) {
    hints.push(
      "当前运行时工作目录指向 /var/task（Vercel/Lambda 风格一次性任务目录），其下 data 文件夹默认不存在/不可写。" +
      "可设置 DB_PATH 环境变量指向持久化目录，或在桌面版切换到「本地环境」运行；预览模式下我们已自动降级到 /tmp。"
    );
  }
  if (/no such directory|ENOENT/i.test(msg) && !hints.length) {
